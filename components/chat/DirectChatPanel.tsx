...
export default function DirectChatPanel({ user, profile, conversationId, peerId, peerName }: Props) {
  const { messages, isLoading, isSending, sendMessage } = useDirectChat(conversationId);
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      >
        <button
          onClick={() => setActivePanel({ type: "profile", userId: peerId })}
          style={{
            background: "none",
            border: "none",
            cursor: "pointer",
        ...
        <div>
          ...
          <p style={{ fontSize: 11, color: "var(--text-muted)" }}>
            Private conversation · Real-time
          </p>
        </div>
        <div
          style={{
